...
def translate(word,data):
    ...
    while True:
        ...
        while state==2: #similar words exists in dictionnary
            ...
            if get_close_matches(word, df.tr.astype(str), 5, 0.3) and get_close_matches(word, df.eng.astype(str), 5, 0.3):
                best_match_tr=get_close_matches(word, df.tr.astype(str), 5, 0.3)[0]
                best_match_eng=get_close_matches(word, df.eng.astype(str), 5, 0.3)[0]
                ...
                if score_tr>score_eng:
                    ...
                    state=4
                    break
            elif len(get_close_matches(word, df.tr.astype(str), 5, 0.3))>0: #similar words exist in tr
                state=3
                print(get_close_matches(word, df.tr.astype(str), 5, 0.3))
                break
            elif len(get_close_matches(word, df.eng.astype(str), 5, 0.3))>0: #similar words exist in EN
                state=4
                print(get_close_matches(word, df.eng.astype(str), 5, 0.3))
                break
            else:
                state=7
                break
        ...
        while state==3: #only case sensitive issue TR
            if word.capitalize() == get_close_matches(word, df.tr.astype(str), 5, 0.3)[0] :
                ...
                state=1
                break
            elif get_close_matches(word, df.tr.astype(str), 5, 0.3)[0] == word.upper():
                word=word.upper()
                state=1
                break
            elif get_close_matches(word, df.tr.astype(str), 5, 0.3)[0] == word.lower():
                word=word.lower()
                state=1
                break
            elif get_close_matches(word, df.tr.astype(str), 5, 0.3)[0] == word.title():
                word=word.title()
                state=1
                break
            else:
        ...
        while state==4: #only case sensitive issue ENG
            if word.capitalize() == get_close_matches(word, df.eng.astype(str), 5, 0.3)[0] :
                word=word.capitalize()
                state=1
                break
            elif get_close_matches(word, df.eng.astype(str), 5, 0.3)[0] == word.upper():
                word=word.upper()
                state=1
                break
            elif get_close_matches(word, df.eng.astype(str), 5, 0.3)[0] == word.lower():
                word=word.lower()
                state=1
                break
            elif get_close_matches(word, df.eng.astype(str), 5, 0.3)[0] == word.title():
                word=word.title()
                state=1
                break
            else:
        ...
        while state==5: #similar words in TR
            ...
            return ["", message ,get_close_matches(word, df.tr.astype(str), 5, 0.3),"",""]
            break

        while state==6: #similar words in ENG
            message=["Bu kelimeyi bulamadık. Aşağıdakilerden birini mi demek istediniz?", "Aradığınızı bulamadınız mı? Bu kelimeye bir çeviri önerin."]
            return ["", message ,get_close_matches(word, df.eng.astype(str), 5, 0.3),"",""]
            break
